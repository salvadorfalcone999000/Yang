...
- Сначала кратко уточни 2-4 ключевых вопросов: бюджет, для чего ПК
...
def ask_openrouter(messages):
    """Отправляет историю диалога в OpenRouter и возвращает ответ."""
    if not openrouter_client:
        return "ИИ временно недоступен. Проверь ключ OPENROUTER_API_KEY."
    ...
    try:
        ...
        return "Извини, произошла техническая ошибка при обращении к ИИ. Попробуй ещё раз."
    ...
    if data == "menu_ai_config":
        if not openrouter_client:
            bot.edit_message_text(
                "ИИ-подбор временно недоступен (не настроен ключ OpenRouter). "
...
def handle_text(message):
    ...
    if step == "ai_chat":
        ...
        try:
            reply = ask_openrouter(state["ai_history"])
        except Exception as e:
